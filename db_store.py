...
def _safe(default=None) -> Callable:
    ...
    def deco(fn: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                log.error("store.%s failed: %s", fn.__name__, e)
                return default
        return wrapper
    return deco
...
class Store:
    ...
    def get_thread(self, key: str) -> Optional[ThreadState]:
        with self._conn() as conn:
            ...
            if row is None:
                return None
            conn.execute("UPDATE threads SET last_used = ? WHERE key = ?",
                         (time.time(), key))
    ...
    def save_thread(self, key: str, thread: ThreadState, cap: int = 1024) -> None:
        ...
        with self._conn() as conn:
            ...
            """, (
                key, thread.backend_uuid, thread.read_write_token,
                thread.slug, thread.title, thread.model, thread.mode,
                thread.account_id, now,
            ))
    ...
    @_safe()
    def save_thread_by_answer(self, answer_hash: str, thread: ThreadState,
                              cap: int = 1024) -> None:
        now = time.time()
        thread.last_used = now
        with self._conn() as conn:
            conn.execute("""
                ...
                ON CONFLICT(answer_hash) DO UPDATE SET
                    ...
                    last_used = excluded.last_used
            """, (
                answer_hash, thread.backend_uuid, thread.read_write_token,
                thread.slug, thread.title, thread.model, thread.mode,
                thread.account_id, now,
            ))
    ...
    @_safe()
    def save_response(self, resp_id: str, data: dict, cap: int = 512) -> None:
        thread: Optional[ThreadState] = data.get("_thread")
    ...
    @staticmethod
    def _evict(conn: sqlite3.Connection, table: str, pk: str,
               order_col: str, cap: int) -> None:
    ...
    def count_responses(self) -> int:
        with self._conn() as conn:
            return conn.execute("SELECT COUNT(*) AS c FROM responses").fetchone()["c"]
